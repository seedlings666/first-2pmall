...
class GoodsController extends Controller
{
    ...
    public function getShow(Application $wechat_app, Request $request, $goods_id)
    {
        ...
        if (is_array($goods_details) && isset($goods_details['err_code'])) {
            return view('wap.error', ['error' => $goods_details]);
            // return Response::json($goods_details);
        }
        ...
        if (!isset($goods_info['is_on_sale']) || $goods_info['is_on_sale'] != 1) {
            //exit('该商品不上架!');
            $err_msg = Helper::ErrorMessage(50010, '该商品已下架!');
            return view('wap.error', ['error' => $err_msg]);
            // return $err_msg;
        }
        ...
        if ($goods_info['is_sku'] != 1) {
            ...
            $default_sku_info = $GoodsModule->getSkuByAttr($goods_id, 0, 0);
            if (isset($default_sku_info['err_code'])) {
                return view('wap.error', ['error' => $default_sku_info]);
                // return Response::json($default_sku_info);
            }
            //当不存在,抛出一个商品数据有误的错误
            if (empty($default_sku_info)) {
            ...
            if (isset($color_attr_arr['err_code'])) {
                return view('wap.error', ['error' => $color_attr_arr]);
                // return Response::json($color_attr_arr);
            }
...
//            return $color_attr_arr;
            ...
            if (isset($size_attr_arr['err_code'])) {
                return view('wap.error', ['error' => $size_attr_arr]);
        ...
        if ($group_id) {
            ...
            $groupOrder = $order->where('id', $group_id)
                ...
                ->first();
            if ($groupOrder && !$groupOrder->canJoin()) {
                $group_id = 0;
                $groupEnd = true;
            }
        }
        ...
        return View::make('wap.index')->with($goods_details)
            ->with(['groupEnd' => $groupEnd, 'group_id' => $group_id])
            ->with(['color_attr' => $color_attr_arr, 'size_attr' => $size_attr_arr])
            ->with(['default_sku_info' => $default_sku_info, 'request' => $request]);
    }
